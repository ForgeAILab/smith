...
completed_at: 2026-08-09T18:17:26Z
---

## 1. Coordinate and pin Agent Runtime contracts

- [x] 1.1 Approve, implement, and land
  ...
  insufficient.
- [x] 1.2 Complete and archive `add-prompt-cache-miss-visibility-2026-08-08`,
...
- [x] 1.3 Pin every Smith Agent Runtime workspace dependency and lockfile entry
...
- [x] 1.4 Add a bounded compatibility alias from `automatic-prefix` to
  `implicit-prefix` and reject ambiguous per-model cache declarations.

## 2. Resolve cache policy and provider capabilities

- [x] 2.1 Add layered `[profiles.<name>.context.cache]` configuration with
...
- [x] 2.2 Treat `context.idle_compaction_ms` as a deprecated one-release alias
  for `context.cache.inactivity_limit_ms`; reject conflicting same-layer values
  and document migration.
- [x] 2.3 Keep `cache.miss_notices` presentation-only and prove it cannot alter
  provider requests, maintenance scheduling, or cache state.
- [x] 2.4 Resolve the requested policy against provider/model capability,
...
- [x] 2.5 Extend built-in and fake provider model profiles with cache-contract
...
- [x] 3.1 Add an exact identity wrapper that consumes Agent Runtime's opaque
...
- [x] 3.2 Add the evidence-bearing lease state, guaranteed-retention metadata,
  meaningful-activity/cache-touch clocks, parked-interval identity,
  maintenance counters, and suspension reason.
- [x] 3.3 Consume Agent Runtime's canonical plan, attempt, usage, cache-state,
...
- [x] 3.4 Retire applicability on provider, endpoint, model/profile, adapter,
...
- [x] 3.5 Add fake-clock tests for guarantee passage without invented expiry,
...
- [x] 4.1 Add one cancellable scheduler per active lease that rechecks exact
...
- [x] 4.2 Implement conformance-gated ephemeral keepalive requests with exact
  prefix identity, identity-bound stable tool schemas preserved, tool choice
  forced to none, no execution, bounded suffix/output/deadline, jitter, no
  canonical history, separate purpose, and no automatic retry.
- [x] 4.3 Implement the optional same-model handoff checkpoint, persist its
  ...
  interval's maintenance request. Consume Runtime's protected live-only text;
  if a crash loses it before capsule persistence, accept the persisted Runtime
  completion without provider replay and preserve cold-resume correctness
  without the optional summary.
- [x] 4.4 Suppress scheduled maintenance after a real matching parent request;
  record a reason without provider I/O.
- [x] 4.5 On an observed maintenance miss, typed upstream expiry/resource
...
- [x] 4.6 Prove a different provider/model summary is ordinary compaction and
  never touches the parent lease.

## 5. Park parents and admit child completion safely

- [x] 5.1 Extend the model-facing `agent.wait` schema with optional
...
- [x] 5.2 Record `parked-awaiting-child` only when at least one direct child
...
- [x] 5.3 Preserve terminal child outcomes through the protected lossless
  channel with deterministic ordering; progress may remain coalescable.
- [x] 5.4 After terminal injection, call Agent Runtime's
...
- [x] 5.5 Serialize user, child-completion, and goal admission so real user
  input wins, no competing provider turns start, and outcomes are neither lost,
  duplicated, nor reordered.
- [x] 5.6 Ensure child provider/tool/progress activity does not reset the parent
  inactivity or cache-touch clocks and cannot extend
  `max_hold_while_child_ms`.
- [x] 5.7 Add deterministic parking, wait-timeout, multi-child ordering,
...
- [x] 6.1 Add the versioned resume-capsule projection through existing
  ...
  project file.
- [x] 6.2 Persist exact session/turn, goal/plan, child, mutation, validation,
...
- [x] 6.3 Make the highest compatible committed protected/canonical watermark
...
- [x] 6.4 Update semantic summaries from bounded deltas plus the previous
  summary and exact projection; retain same-model handoff and independent
  summary routes separately.
- [x] 6.5 At the inactivity limit, persist exact state and attempt compaction
...
- [x] 6.6 Prove cold process resume restores canonical state, starts provider
...
- [x] 6.7 Add schema migration, corruption, protection-key, watermark,
  summary-conflict, sensitive-content, and repeated-compaction fixtures.

## 7. Project lifecycle, usage, and status consistently

- [x] 7.1 Consume Agent Runtime's redaction-safe plan, observation, admission,
  ...
  provider vocabulary.
- [x] 7.2 Preserve `cache_keepalive` and add separately attributed
  ...
  outcome.
- [x] 7.3 Count synthetic attempts toward provider/session totals and limits
  without presenting them as user, parent, or child turn usage.
- [x] 7.4 Add lease status, structural preserved tokens, provider reads/writes,
...
- [x] 8.1 Add adapter conformance proving the opaque exact identity and
  ...
  maintenance retry.
- [x] 8.2 Reject any synthetic response tool call without execution and record
  the provider contract violation.
- [x] 8.3 Stop scheduling first on shutdown, cancel in-flight synthetic work,
...
- [x] 8.4 Document configuration, authority narrowing, lease/evidence
...
- [x] 8.5 `cargo fmt --all --check`.
- [x] 8.6 `cargo clippy --workspace --all-targets -- -D warnings`.
- [x] 8.7 `cargo test --workspace`.
- [x] 8.8 Run strict spec validation for
