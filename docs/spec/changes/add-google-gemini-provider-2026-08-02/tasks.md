...
updated_at: 2026-08-04T02:41:36Z
completed_at:
---
...
- [x] 0.3 Approve and implement the upstream change before exposing the adapter
  in Smith.
- [x] 0.4 Rebase onto the final provider-catalog and `/connect` contracts
...
- [x] 2.1 Add `google` to the bounded Models.dev generator/runtime allow-list
  and regenerate the embedded seed reproducibly.
- [x] 2.2 Bind the built-in `gemini-interactions` adapter to the Google catalog
  without importing endpoint, npm package, environment, or auth policy.
- [x] 2.3 Add a trusted Google setup/connection descriptor with fixed endpoint,
  API-key methods, and `gemini-3.6-flash` as only the recommended model ID.
- [x] 2.4 Add catalog absence/corruption, invalid model, explicit override,
  provenance, and no-generated-model-record tests.

## 3. Native runtime composition

- [x] 3.1 Consume the approved Agent Runtime release and add
  `gemini-interactions` to Smith's compiled adapter inventory and factory.
- [x] 3.2 Enforce the fixed trusted endpoint, `store=false`, streaming,
...
- [x] 4.1 Expose Google Gemini in guided setup and `/connect google` using the
  existing masked, reviewed API-key transaction.
- [x] 4.2 Populate `/model` from the same frozen Google catalog used by runtime
  preflight, with catalog provenance and disabled reasons.
- [x] 4.3 Ensure catalog-backed setup writes no endpoint or model metadata and
