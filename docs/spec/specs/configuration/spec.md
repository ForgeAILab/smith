...
a default of `true`. The setting SHALL control only local human-facing miss
...
- **THEN** significant miss notices are enabled
- **AND** cache state remains available through status and machine output

#### Scenario: User disables notices

- **GIVEN** user configuration sets `cache.miss_notices = false`
- **WHEN** Smith explains and runs the resolved configuration
- **THEN** explain output names the user source and disabled value
- **AND** qualifying completed turns append no local miss notice
