...
Smith SHALL persist the fact that a monitor or child was active, but MUST NOT
claim that the work survives process exit. On resume after a crash, previously
active work MUST be marked `interrupted_by_process_exit` and MUST NOT restart
automatically.
...
Smith MUST warn or return an active-work error before normal exit when monitors
or children remain. Once exit is confirmed, it SHALL stop accepting new work,
cancel children, terminate owned process groups, append terminal events, flush
canonical records, and exit within a bounded grace period.
