...
use smith_config::resolve::{resolve, ResolveRequest};
...
fn default_threshold_is_independent_of_the_legacy_output_limit() {
    ...
    std::fs::write(project.path().join(".smith/config.toml"), format!("{BASE}\n[limits]\ntool_output_limit_bytes = 131072\n")).unwrap();
...
fn configured_threshold_is_layered_and_environment_provenance_is_preserved() {
    ...
    assert_eq!(resolve(&request).unwrap().config.context.tool_output_inline_bytes.value, 16384);
    let result = resolve(&request.with_env([("SMITH_CONTEXT_TOOL_OUTPUT_INLINE_BYTES", "4096")])).unwrap();
    assert_eq!(result.config.context.tool_output_inline_bytes.value, 4096);
    assert_eq!(result.config.context.tool_output_inline_bytes.source.key, "SMITH_CONTEXT_TOOL_OUTPUT_INLINE_BYTES");
}
...
fn unsafe_or_mistyped_thresholds_are_rejected_at_configuration_resolution() {
    ...
    for invalid in ["0", "255", "1048577", "-1", "\"8192\"", "true"] {
        ...
        assert!(resolve(&ResolveRequest::new(project.path()).with_home_dir(home.path())).is_err(), "{invalid}");
    }
}
